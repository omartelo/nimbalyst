...
### Added
- Filter the sessions list by tag with a `#` picker. Typing `#` in the sessions list search opens a tag typeahead with workspace tag counts; selected tags become removable chips. Title search and tag filter compose (AND across kinds, OR within tags), matching the Kanban filter behaviour. The match-count below the search now reads "X of Y sessions" when any filter is active so users can see how many results their filter cut. (#244)
- In-app ChatGPT / API-key sign-in for OpenAI Codex via the app-server transport. Codex `account/*` RPCs are now driven from a lazy long-lived auth child so users can sign in to ChatGPT or save an API key without leaving Nimbalyst, replacing the old "open Terminal and run `codex login`" flow. A pre-flight auth gate before app-server session start surfaces a "Sign in to OpenAI Codex" CTA in the transcript with a jump-to-settings button, instead of letting the child hang on a silent reconnect loop when `~/.codex/auth.json` is missing. `refreshToken: true` is forced and the gate keys on `account === null` so a stale in-memory view in the long-lived child can't mis-report a signed-in user as signed out. Captured codex stderr tail is appended to `turn/failed` and `error` events for diagnostics.
- First-class URL fields for tracker items and PR metadata in the tracker schema; new workspace tracker schemas are loaded immediately in main and renderer.
- Compact Rename and Move buttons surfaced below the primary project actions in the project header. The existing context-menu actions still work; rename vs. move progress messaging in the operation overlay is clarified. (Refs #305)

### Changed
- Agent session transcript switched to a flat `content-visibility:auto` list on non-iOS hosts so DOM node identity survives scrolling and idle metadata churn. Drag-selections used to be destroyed whenever virtua unmounted rows; stable per-message and per-tool keys (derived from message id, sequence, providerToolCallId, or fallback index) plus the new layout preserve text selection across reloads, scroll, and metadata changes. `AgentTranscriptPanel` memo no longer re-renders on every metadata reference change; only `sessionStatus` and `currentTeammates` trigger re-renders. iOS WKWebView keeps the existing VList path because selection is rarely used there and WebKit's `content-visibility` support is younger. Chat bottom uses `margin-top:auto` so short transcripts hug the input instead of floating at the top; a bottom sentinel + `scrollIntoView` gives reliable scroll-to-bottom even when `content-visibility` hasn't measured every row yet.
...
- Long multi-line git errors no longer blow up the cross-tab status bar to fill the whole panel. The status bar now stays a single line with ellipsis and adds View full error + Copy buttons. Extracted into a reusable `GitStatusBar` component used by both the log and changes tabs; status bar truncates to the first non-empty line with full text on hover title; View full error opens the existing scrollable popup; Copy puts the raw error on the clipboard; the failing command (git push / pull / fetch) is threaded into the popup header.
- Git extension Output tab errors are now readable and copyable. Error preview truncates to 3 lines with a "+N more lines" indicator; a "View full error" button opens a scrollable modal popup; a "Copy" button copies the raw error to the clipboard.
- Codex pasted images unblocked in app-server turns. The v2 app-server schema expects `localImage` (camelCase) for image inputs; Nimbalyst was sending `local_image`, causing the server to reject any turn that included a pasted image with RPC -32600 "unknown variant" and surface a red error in the transcript.
- Codex helper PATH preserved in app-server mode. Nimbalyst now mirrors the official Codex launcher when spawning app-server, preventing packaged Windows builds from losing vendored helpers.
- Codex `developer_git_commit_proposal` no longer produces duplicate "Changes Committed" cards. A Codex session that resumed across SDK / app-server transports produced two `tool_call` canonical events for the same proposal call, rendering two side-by-side widgets — one without the commit hash or timestamp. `GitCommitConfirmationWidget` now parses JSON-string results so the structured `{action, commitHash, commitDate}` payload renders the hash badge and timestamp; `processDescriptor` treats synthetic `nimtc|...` edit-group IDs as per-call and suppresses a second `tool_call_started` even after the first event has completed; `OpenAICodexProvider` warns when `storeRawEventIfPresent` writes an SDK-shape `raw_event` during app-server transport, with a short stack so the upstream emitter is locatable next time.
- Codex bottom-left usage indicator no longer sticks at a stale percentage (e.g. 91%) across panel refresh, app relaunch, or machine restart. `CodexUsageService.extractRateLimitsFromEvent` was returning the most recent token_count event from `~/.codex/sessions/*.jsonl` whenever its primary block was non-null, with no check on `resets_at`. After the 5-hour or 7-day window reset, the same JSONL line kept matching the scan-backward loop, so the cold-start `refresh()` re-read the same stale file each time. A new pure `filterRateLimitsByExpiry` helper drops each bucket independently when its `resets_at` Unix-seconds timestamp is `<= now`. A still-active 7-day window keeps surfacing even when the 5-hour window just reset; if both are expired the function returns null so the scan continues into older lines / files for a still-active block. If nothing is active anywhere the snapshot falls through to `limitsAvailable: false` and the renderer shows `--` instead of a stale percentage. (#120)
- "Thinking…" indicator no longer stays pinned after Claude Code finishes responding on extended-thinking Opus sessions (or any session where a startup race / HMR re-eval leaves the renderer without a workspacePath for the sessionId). Terminal lifecycle events (`session:completed` / `error` / `interrupted`) now clear `sessionProcessingAtom` and `sessionHasPendingInteractivePromptAtom` unconditionally before the `ownedWorkspacePath` null-guard added in v0.60.0's multi-project rail routing fix; `clearSessionStreamingAtom` also runs before the early-return so the multi-project rail's "streaming" badge for inactive projects clears too. The atom is keyed only by `sessionId` (no workspace component), so a missing workspacePath is no reason to keep it true. The workspace-routed branches below (safety-net reload, bucket cleanup) still run only when the path is known. (#116)
- PGLite lock self-heals on Windows after PID reuse. The lock-liveness check used `process.kill(lockPid, 0)` and interpreted any `EPERM` error as "still running." On Windows, `process.kill(pid, 0)` against a PID owned by another user / privilege level throws `EPERM` regardless of whether that PID is the original Nimbalyst process or a totally unrelated system / service process that recycled the number. Windows PID namespace is 16-bit and recycling happens aggressively within a session, especially after 3+ hours of activity. New pure helper `decideLockIsRunning` (`packages/electron/src/main/database/lockStaleness.js`) adds a 60-second grace window inside the `EPERM` branch: a genuine competing Nimbalyst instance acquires its lock during the current launch attempt with a seconds-fresh timestamp; a crash-stale lock has a timestamp from before the crash (minutes to hours old) and is correctly classified stale. (#272)
- Quick Open and the @-mention picker no longer flash stale hover state when opened via keyboard. List `pointer-events` are gated on `mouseHasMoved` across all four Quick Open menus so a stationary cursor over a row doesn't show hover state before the user actually moves the mouse. `PromptQuickOpen` also gains Cmd+Enter copy with a transient "Copied to clipboard" toast, memoized `PromptRow` rows with stable callbacks (render commit ~160ms -> few ms at N=2000), a "Loading…" state during prompt fetch instead of flashing "No recent prompts", and the provider icon moved inline beside the session title in the meta row (each row is a prompt, not a session).
- Agent session history list now has stable ordering. Agent-mode session history sorts from turn boundaries; reorder commits are throttled to avoid rapid list flicker; tie-breaking is stable for concurrent sessions.
- Collab v3 JWT-mismatch warning is now actionable. The warning decodes the JWT `organization_id` claim so it can state plainly whether the JWT is team-scoped vs. personal-org-scoped (instead of "may be team-scoped"); shows all four facts (`jwtSub`, `jwtOrgId`, `configUserId`, `configOrgId`) with inline labels naming `personalUserId` / `personalOrgId`; replaces "using config.userId for room routing" framing with the actual consequence ("server will reject the WebSocket auth"); and points at the concrete next file to look at (`refreshPersonalSession` if JWT is team-scoped, `resolvePersonalUserId` if user IDs disagree within the same org). `extractUserIdFromJwt` renamed to `decodeJwtClaims`.
